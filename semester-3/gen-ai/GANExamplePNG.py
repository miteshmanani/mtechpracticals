...
# Preprocess and standardize images (support PNG)
...
def preprocess_images(input_dir, output_dir):
    ...
    print(f"Found {len(input_files)} PNG files in {input_dir}")

    if not input_files:
        raise ValueError(
            f"No PNG files found in {input_dir}. Please check the directory.")
...
class ImageDataset(Dataset):
    def __init__(self, directory):
        ...
        print(f"Found {len(self.images)} JPG files in {directory}")
        if not self.images:
            raise ValueError(
                f"No JPG files found in {directory}. Preprocessing may have failed.")
