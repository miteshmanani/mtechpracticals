...
# Hyperparameters
TRAIN_DIR = r"C:\mtechpracticalsdatasets\horse2zebra"
VAL_DIR = r"C:\mtechpracticalsdatasets\horse2zebra"
BATCH_SIZE = 4
LEARNING_RATE = 1e-5
LAMBDA_CYCLE = 10
NUM_WORKERS = 4  # Adjust based on your CPU cores; 4 is typically good for most systems
NUM_EPOCHS = 50
LOAD_MODEL = False
SAVE_MODEL = True
CHECKPOINT_DIR = os.path.join(TRAIN_DIR, "models")
os.makedirs(CHECKPOINT_DIR, exist_ok=True)
CHECKPOINT_GENERATOR_H = os.path.join(CHECKPOINT_DIR, "genh.pth.tar")
CHECKPOINT_GENERATOR_Z = os.path.join(CHECKPOINT_DIR, "genz.pth.tar")
CHECKPOINT_DISCRIMINATOR_H = os.path.join(CHECKPOINT_DIR, "disch.pth.tar")
CHECKPOINT_DISCRIMINATOR_Z = os.path.join(CHECKPOINT_DIR, "discz.pth.tar")
...
class ResidualBlock(nn.Module):
    def __init__(self, channels: int):
        ...
        self.block = nn.Sequential(
            ConvolutionalBlock(channels, channels,
                               add_activation=True, kernel_size=3, padding=1),
            ConvolutionalBlock(channels, channels,
                               add_activation=False, kernel_size=3, padding=1),
        )
...
class Generator(nn.Module):
    def __init__(self, img_channels: int, num_features: int = 64, num_residuals: int = 9):
        ...
        self.upsampling_layers = nn.ModuleList([
            ...
            ConvolutionalBlock(num_features * 2, num_features * 1, is_downsampling=False,
                               kernel_size=3, stride=2, padding=1, output_padding=1),
        ])

        self.last_layer = nn.Conv2d(num_features * 1, img_channels, kernel_size=7,
...
# ConvInstanceNormLeakyReLU Block
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=3, features=[64, 128, 256, 512]):
        ...
        for feature in features[1:]:
            layers.append(
                ConvInstanceNormLeakyReLUBlock(in_channels, feature,
    ...
    for idx, (zebra, horse) in enumerate(loop):
        ...
        if idx % 200 == 0:
            ...
            save_image(fake_horse * 0.5 + 0.5,
                       os.path.join(output_dir, f"horse_{idx}.png"))
            save_image(fake_zebra * 0.5 + 0.5,
                       os.path.join(output_dir, f"zebra_{idx}.png"))
...
def main():
    ...
    if LOAD_MODEL:
        for checkpoint, model, optimizer in [
            (CHECKPOINT_GENERATOR_H, gen_H, opt_gen),
            (CHECKPOINT_GENERATOR_Z, gen_Z, opt_gen),
            (CHECKPOINT_DISCRIMINATOR_H, disc_H, opt_disc),
            (CHECKPOINT_DISCRIMINATOR_Z, disc_Z, opt_disc)
        ]:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        train_fn(disc_H, disc_Z, gen_Z, gen_H, loader, opt_disc, opt_gen,
                 L1, mse, d_scaler, g_scaler)

        if SAVE_MODEL:
            save_checkpoint(gen_H, opt_gen, CHECKPOINT_GENERATOR_H)
            save_checkpoint(gen_Z, opt_gen, CHECKPOINT_GENERATOR_Z)
            save_checkpoint(disc_H, opt_disc, CHECKPOINT_DISCRIMINATOR_H)
            save_checkpoint(disc_Z, opt_disc, CHECKPOINT_DISCRIMINATOR_Z)
